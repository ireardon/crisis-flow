...
function getMessagesForRoom(room_id, limit, callback) {
	...
	var sql;
	if(limit) {
		sql = 'SELECT * FROM messages WHERE room = $room_id AND reply < 0 ORDER BY time DESC LIMIT $limit';
	} else {
		sql = 'SELECT * FROM messages WHERE room = $room_id AND reply < 0 ORDER BY time DESC';
	}
