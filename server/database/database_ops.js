...
module.exports = {
	...
	'createTaskFollowup': createTaskFollowup,
	...
	'attachFilesToTask': attachFilesToTask
};
...
function createTaskFollowup(task, content, author, callback) {
	...
	dataConn.run(sql, {
		...
		report.error(error, 'dbops.createTaskFollowup failed');
...
function getAdditionalsForTasks(tasks, callback) {
	getAttachmentsForTasks(0, tasks, function(error, tasksWithAttachments) {
		if(error) {
			callback(error);
			return;
		}
		...
		getTagsForTasks(0, tasksWithAttachments, function(error, tasksWithAttachmentsAndTags) {
			...
			getFollowupsForTasks(0, tasksWithAttachmentsAndTags, function(error, tasksWithAttachmentsAndTagsAndFollowups) {
				if(validCallback(callback)) {
...
function getFollowupsForTask(taskID, callback) {
	...
	dataConn.all(sql, {$taskID: taskID}, function(error, rows) {
		report.error(error, 'dbops.getFollowupsForTask failed');
